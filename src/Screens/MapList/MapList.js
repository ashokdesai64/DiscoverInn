...
class MapList extends React.Component {
  ...
  shareToSocial(type) {
    ...
    if (type == 'facebook') {
      Share.shareSingle({
        ...
        .catch(err => {
          this.setState({ shareModal: false });
          alert(`Can't share to ${type} because, ${err}`);
        });
    } else {
      Share.shareSingle({
  ...
  findUsers(query) {
    ...
    return allUserNames.filter(user => user.username.search(regex) >= 0 && !selectedUserNames.find(u => u.id == user.id));
  }
  ...
  shareMap() {
    ...
    if (shareSelectedMap.id && (selectedUserNames.length > 0 || selectedUserEmails.length > 0)) {
      this.props.mapAction.shareMap({
        user_id: this.props.userData.id,
        ...
        this.setState({ shareModal: false, selectedUserNames: [], query: '', sharingMap: false })
      }).catch((err) => {
    ...
    return (
      <Fragment>
        ...
        <Dialog
          ...
          animationDuration={1}
          onTouchOutside={() => {
            this.setState({ shareModal: false, selectedUserNames: [], query: '' });
          }}
          dialogAnimation={
            new FadeAnimation({
              initialValue: 0, // optional
          ...
          <DialogContent style={styles.customPopupContent}>
            <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps={'always'}>
              <View style={styles.customPopupHeader}>
                ...
                <TouchableOpacity
                  style={styles.buttonClose}
                  onPress={() => this.setState({ shareModal: false, selectedUserNames: [], query: '' })}>
                  <Feather name={'x'} style={styles.buttonCloseIcon} />
                </TouchableOpacity>
              </View>
              <View>
              ...
              <View style={[styles.formGroup, { marginBottom: 5 }]}>
                ...
                <AutoComplete
                  ...
                  data={userNames.length === 1 && comp(query, userNames[0].username) ? [] : userNames}
                  defaultValue={query}
                  onChangeText={text => this.setState({ query: text })}
                  placeholder="Enter Name"
                  renderItem={({ item, i }) => (
                    <TouchableOpacity style={{ flex: 1, margin: 5, padding: 5 }} onPress={() => this.selectedShareUserName(item)}>
                      <Text style={{ fontFamily: 'Montserrat-Medium', }}>{item.username}</Text>
                    </TouchableOpacity>
                  )}
                />
              </View>
              <View style={{ flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'flex-start', alignItems: 'center', marginBottom: 10 }}>
                {
                  selectedUserNames && selectedUserNames.length > 0 && selectedUserNames.map((user) => {
                    return (
                      <View style={{ height: 30, flexDirection: 'row', backgroundColor: '#ccc', borderRadius: 20, justifyContent: 'center', alignItems: 'center', paddingHorizontal: 5, margin: 5 }}>
                        <Text style={{ fontFamily: 'Montserrat-Medium', marginRight: 10 }}>{user.username}</Text>
                        <TouchableOpacity onPress={() => this.removeSelectedUserName(user)} style={{ backgroundColor: '#aaa', height: 20, width: 20, borderRadius: 10, justifyContent: 'center', alignItems: 'center' }}>
              ...
              <View style={[styles.formGroup, { marginBottom: 5 }]}>
                ...
                <TextInput
                  style={[styles.formControl, { borderColor: this.state.wrongEmail ? 'red' : '#BDBDBD' }]}
                  placeholder={'Enter Email'}
                  placeholderTextColor={'#828894'}
                  returnKeyType={'done'}
                  onChangeText={(registeredEmail) => this.setState({ registeredEmail })}
              ...
              <View style={{ flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'flex-start', alignItems: 'center', marginBottom: 10 }}>
                {
                  selectedUserEmails && selectedUserEmails.length > 0 && selectedUserEmails.map((email) => {
                    return (
                      <View style={{ height: 30, flexDirection: 'row', backgroundColor: '#ccc', borderRadius: 20, justifyContent: 'center', alignItems: 'center', paddingHorizontal: 5, margin: 5 }}>
                        <Text style={{ fontFamily: 'Montserrat-Medium', marginRight: 10 }}>{email}</Text>
                        <TouchableOpacity onPress={() => this.removeSelectedEmail(email)} style={{ backgroundColor: '#aaa', height: 20, width: 20, borderRadius: 10, justifyContent: 'center', alignItems: 'center' }}>
                          <Feather name={'x'} color={'white'} />
                        </TouchableOpacity>
                      </View>
                    )
